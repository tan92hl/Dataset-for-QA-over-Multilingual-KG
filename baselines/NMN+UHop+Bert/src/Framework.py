...
import numpy
import math
from data_utilities import PerQuestionDataset, random_split, quick_collate, PerQuestionDatasetMLPQ, ACC_COUNT
...
class Framework():
    def __init__(self, args, word2id, rela2id, vocab_mapping, bert_tokenizer):
    ...
    def _set_optimizer(self, model):
        ...
        params = [
            ...
            {"params": model.bert_model.parameters(), "lr": self.args.learning_rate_bert},
        ]
        if self.args.optimizer == 'adam':
            self.optimizer = torch.optim.Adam(params, weight_decay=self.args.l2_norm, amsgrad=True)
        if self.args.optimizer == 'rmsprop':
            self.optimizer = torch.optim.RMSprop(params, weight_decay=self.args.l2_norm)
        if self.args.optimizer == 'adadelta':
            self.optimizer = torch.optim.Adadelta(params, weight_decay=self.args.l2_norm)
#        return optimizer
    ...
    def train(self, model):
        # prepare dataset
        dataset = PerQuestionDatasetMLPQ(self.args, 'train', self.word2id, self.rela2id, self.vocab_mapping, self.bert_tokenizer)
        if self.args.dataset.lower() == 'wq' or self.args.dataset.lower() == 'wq_train1test2':
            ...
            valid_dataset = PerQuestionDatasetMLPQ(self.args, 'valid', self.word2id, self.rela2id, self.vocab_mapping, self.bert_tokenizer)
        ...
        for epoch in range(0, self.args.epoch_num):
            ...
            valid_loss, valid_acc, valid_score, _ = self.evaluate(model, None, None, 'valid', valid_dataset)
    ...
    def evaluate(self, model, bert_model, bert_config, mode, dataset, output_result=False):
        if model == None:
            model = self.args.Model(self.args, bert_model, bert_config).cuda()
            model = load_model(model, self.args.path)
        model = model.eval().cuda()
        # for k, v in model.bert_model.named_parameters():
        #     print(k)
        #     print(v)
        #     print("--------------------------------------------------------------------------------")
        if dataset == None:
            dataset = PerQuestionDatasetMLPQ(self.args, mode, self.word2id, self.rela2id, self.vocab_mapping, self.bert_tokenizer)
        ...
        acc_count = ACC_COUNT[self.args.dataset]
        ...
        #dump result
        if output_result:
            with open(f'{self.args.path}/{self.args.dataset}_scores_{100*total_acc/acc_count:.2f}.json', 'w') as f:
                json.dump(scores, f, indent=2)
            with open(f'{self.args.path}/{self.args.dataset}_prediction.txt', 'w') as f:
                f.write('\n'.join(labels))
