...
class Model(nn.Module):
    def __init__(self, args, bert_model, bert_config):
        ...
        self.linear1 = nn.Linear(self.bert_config.hidden_size, args.hidden_size * 2, bias=True)
    ...
    def forward(self, *inputs):
        ...
        if(self.q_representation=="bert"):
            ...
            ques_hs1, pooled_question = self.bert_model(input_ids=question, attention_mask=ques_mask, token_type_ids=ques_segment)
            # print(question)
            # print(ques_hs1)
            # exit()
            ques_hs1 = self.dropout(ques_hs1)
            ques_hs = th.transpose(ques_hs1, 0, 1)
            ques_hidden_state = None
        else:
